...
with st.form(key="search_form"):
    ...
    # 교원방식 선택 제목
    st.markdown("<div style='font-weight: 600; font-size: 16px; margin-top:10px;'>✅ 교원방식 선택</div>", unsafe_allow_html=True)
...
if submitted:
    if keyword:
        ...
        results['정확도점수'] = results['검색_부문'].apply(compute_score)
    ...
    # 결과 표시
    st.markdown(f"### 🔎 '{keyword if keyword else '모든'}' 관련 추천 교원과정: {len(results)}건")
    ...
    if results.empty:
        ...
        for _, row in results.iterrows():
            ...
            with st.container():
                st.markdown(f"##### 📘️ {row['과정명']} (정확도: {row['정확도점수']}점)")
