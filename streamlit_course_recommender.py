...
# 페이지 설정: 전체 화면(넓은 레이아웃) 사용 및 좌우 여백 지정
st.set_page_config(
...
results = df.copy()
if submitted:
    # 교육방식 필터링: 선택된 교육방식에 따라 필터 적용 (키워드 없이도 해당 조건 적용)
    ...
    if results.empty:
        ...
        <head>
          ...
          <script>
             function slideRight(containerId) {
                 ...
                 var cardWidth = 320; // 카드의 너비 + 오른쪽 마진 (300 + 20)
        ...
        <body>
        """
        # 각 대분류별로 그룹을 묶어 카드 슬라이더 생성
        for cat in category_order:
            group = results[results['대분류'] == cat]
            if not group.empty:
                # 그룹별 슬라이더 id 생성 (문자열에서 특수문자 제거)
                slider_id = f"slider_{''.join(e for e in cat if e.isalnum())}"
        ...
        """
        # st.components.v1.html()를 이용해 HTML 슬라이더 렌더링 (높이는 필요에 따라 조정)
